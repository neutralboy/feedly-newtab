...
const AllCaughtUp = ({ loading, allCaughtUp, cardBg, bg, id }: IAllCaughtUp) => {
    ...
    const dLoadMore = () => {
        ...
        if(state.accessToken){
            loadMore(state.user, state.accessToken)
        };
    };

    return (
        <div className={`${cardBg} rounded-md shadow-lg w-1/3 mx-auto p-6 px-8`}>
            {
                (dLoading||loading) && 
            ...
            <div className="mx-auto text-center" >
                ...
                <button onClick={dLoadMore} className={`text-gray-500 mt-3 hover:${bg} px-3 py-2`} >
